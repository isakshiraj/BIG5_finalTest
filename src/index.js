...
app.post("/save-result", async (req, res) => {
    const { username, topPersonality1, topPersonality2 } = req.body;

    try {
        const user = await collection.findOneAndUpdate(
            { name: username },
            { $push: { results: { topPersonality1, topPersonality2, date: new Date() } } },
            { new: true }
        );
...
app.get('/download-results-pdf', async (req, res) => {
    ...
    try {
        ...
        user.results.forEach(result => {
            doc.fontSize(12).text(new Date(result.date).toLocaleDateString('en-GB'), { continued: true }).text(result.topPersonality1, { continued: true, align: 'center' }).text(result.topPersonality2, { align: 'right' });
            doc.moveDown();
        });
...
// Serve Big5Test directory as static content
app.use('/big5test', express.static(path.join(__dirname, '../Big5Test')));

// Serve Big5Test index.html
app.get('/big5test', (req, res) => {
    res.sendFile(path.join(__dirname, '../Big5Test/index.html'));
});
